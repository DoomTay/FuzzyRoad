...
public class CarCont3 : MonoBehaviour {
	...
	public void KeyboardControlling (){
		...
		//Motor Input.
		if(!changingGear)
			motorInput = (Input.GetAxis("Vertical3"));
		else
			motorInput = Mathf.Clamp(Input.GetAxis("Vertical3"), -1f, 0f);

		//Steering Input.
		if(Mathf.Abs (Input.GetAxis("Horizontal3")) > .05f)
			steerInput = Mathf.Lerp (steerInput, Input.GetAxis("Horizontal3"), Time.deltaTime * 10);
		else
			steerInput = Mathf.Lerp (steerInput, Input.GetAxis("Horizontal3"), Time.deltaTime * 10);
