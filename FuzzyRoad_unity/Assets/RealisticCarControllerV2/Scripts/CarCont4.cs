...
public class CarCont4 : MonoBehaviour {
	...
	public void KeyboardControlling (){
		...
		//Motor Input.
		if(!changingGear)
			motorInput = (Input.GetAxis("Vertical4"));
		else
			motorInput = Mathf.Clamp(Input.GetAxis("Vertical4"), -1f, 0f);

		//Steering Input.
		if(Mathf.Abs (Input.GetAxis("Horizontal4")) > .05f)
			steerInput = Mathf.Lerp (steerInput, Input.GetAxis("Horizontal4"), Time.deltaTime * 10);
		else
			steerInput = Mathf.Lerp (steerInput, Input.GetAxis("Horizontal4"), Time.deltaTime * 10);
